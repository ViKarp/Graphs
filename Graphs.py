...
class Line:
    def __init__(self, start, end, weight, arrow_mid, arrow_left, arrow_right):
        self.start = start
        self.end = end
        self.weight = weight
...
class Graph:
    ...
    def add_lines(self, startflag, endflag, prior_mid, prior_left, prior_right):
        L = Line(startflag, endflag, ((self.nodes[startflag].center[0] - self.nodes[endflag].center[0]) ** 2 + (
                    self.nodes[startflag].center[1] - self.nodes[endflag].center[1]) ** 2) ** 0.5, prior_mid, prior_left, prior_right)
        self.lines.append(L)
    ...
    def reimage(self, im, new_graph: nx.Graph):
        for edge in self.nxG.edges:
            if edge not in new_graph.edges and (edge[1], edge[0]) not in new_graph.edges:
                for i in self.lines:
                    if (edge[0] == self.nodes[i.start] and edge[1] == self.nodes[i.end]) or (edge[1] == self.nodes[i.start] and edge[0] == self.nodes[i.end]):
...
def main():
    ...
    col = [[sg.T('Choose what clicking a figure does', enable_events=True)],
           ...
           [sg.Text('max t: '), sg.Spin(values=[i for i in range(50, 100)], initial_value=100, size=(6, 1), key='-MAXT-', readonly=True)],
           [sg.Text('min t: '),
    ...
    G = Graph()
    dragging = False
    start_point = end_point = prior_rect = prior_right = prior_left = prior_mid = None
    startflag = -1
    endflag = -1
    ...
    while True:
        ...
        if event == "-AS-":
            ...
            #window['-SOL-'].update(anneal_common.get_path(graph_sol))
        if event == "-NN-":
            ...
            #window['-SOL-'].update(anneal_common.get_path(graph_sol))
        if event == "-MAXT-":
            anneal_common.args['max_t'] = values['-MAXT-']
        if event == "-MINT-":
            anneal_common.args['min_t'] = values['-MINT-']
        if event == "-MAXT-":
            anneal_common.args['step'] = values['-STEP-']
        if event == "-OPT-":
        ...
        if event == "-GRAPH-":  # if there's a "Graph" event, then it's a mouse
            ...
            if prior_left:
                graph.delete_figure(prior_left)
            if prior_right:
                graph.delete_figure(prior_right)
            ...
            if None not in (start_point, end_point):
                ...
                if values['-CIRCLE-']:
                    ...
                    k = (end_point[1] - start_point[1]) / (end_point[0] - start_point[0] + 0.1)
                    prior_left = graph.draw_line(end_point, arrow((end_point[0] + (
                                400 / (math.tan(math.atan(k) + math.pi / 6) ** 2 + 1)) ** 0.5, end_point[0] - (400 / (
                                math.tan(math.atan(k) + math.pi / 6) ** 2 + 1)) ** 0.5),
                                                                  math.tan(math.atan(k) + math.pi / 6), start_point,
                                                                  end_point), width=4)
                    prior_right = graph.draw_line(end_point, arrow((end_point[0] + (
                                400 / (math.tan(math.atan(k) - math.pi / 6) ** 2 + 1)) ** 0.5, end_point[0] - (400 / (
                                math.tan(math.atan(k) - math.pi / 6) ** 2 + 1)) ** 0.5),
                                                                   math.tan(math.atan(k) - math.pi / 6), start_point,
                                                                   end_point), width=4)

                # TODO создание структуры нодов и стрелок, подключение к networkx
